...
# Bayesian Optimization with Stratified K-Fold
def bayesian_optimization(X, y, init_points=10, n_iter=20, n_splits=5):
    def xgb_cv(colsample_bytree, learning_rate, max_depth, min_child_weight,
...
if __name__ == "__main__":
    ...
    print(f"Best Validation Gini Score: {best_score:.4f}")
    print(f"Best Hyperparameters: {best_params}")

    model_path, gini_test = train_and_save_best_model(X_train, y_train, X_test, y_test, best_params)
    print(f"Final Model Path: {model_path}, Test Gini: {gini_test:.4f}")
